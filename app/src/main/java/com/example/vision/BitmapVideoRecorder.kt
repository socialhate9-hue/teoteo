...
class BitmapVideoRecorder(
    ...
    fun recordFrame(bitmap: Bitmap, overlayData: VideoGameOverlayData? = null) {
        ...
        val canvas = try {
            ...
            Log.w(TAG, "Cannot lock canvas on inputSurface: ${e.message}")
            return
        }
        ...
        try {
            ...
            try {
                ...
                Log.w(TAG, "Failed unlockCanvasAndPost: ${e.message}")
            }
        }
    ...
    fun stop(): File? {
        ...
        release()
        return if (outputFile.exists() && outputFile.length() > 0 && samplesWritten > 0) outputFile else null
    }
    ...
    private fun drainEncoder(endOfStream: Boolean) {
        ...
        var eosWaitRetries = 0
        while (true) {
            val outIndex = try {
                encoder.dequeueOutputBuffer(bufferInfo, 10_000)
            } catch (e: Exception) {
                Log.w(TAG, "Error dequeueing output buffer: ${e.message}")
                break
            }
            ...
            if (outIndex == MediaCodec.INFO_TRY_AGAIN_LATER) {
                if (!endOfStream) {
                    ...
                    eosWaitRetries++
                    if (eosWaitRetries >= 5) {
                        // Max 50ms wait on EOS to prevent hanging/abandoned buffers
                        break
                    }
                }
    ...
    private fun release() {
        // 1. Releasing inputSurface FIRST before codec stops prevents GraphicBufferSource cancelBuffer abandonment
        try {
        ...
        // 3. Stop and release muxer: only call stop() if muxer was started AND samples were written
        // Calling stop() with 0 samples written causes MPEG4Writer: "Stop() called but track is not started or stopped"
        if (muxerStarted) {
            if (samplesWritten > 0) {
                try {
                    muxer?.stop()
                ...
                Log.w(TAG, "Muxer started but 0 samples were written; skipping stop() to avoid MPEG4Writer error")
            }
            muxerStarted = false
        }
        try {
        ...
        // Delete empty recording file if no samples were written
        if (samplesWritten == 0L && outputFile.exists()) {
            try {
                outputFile.delete()
            } catch (_: Exception) {}
        }

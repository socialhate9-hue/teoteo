...
object HighlightVideoGenerator {
    ...
    private fun remuxSegments(
        ...
        try {
            ...
            writeAudioUpTo(currentVideoPtsUs)
            if (videoSamplesWritten > 0 && (muxAudioTrack < 0 || audioSamplesWritten > 0)) {
                try {
                    muxer.stop()
                    Log.i(TAG, "Remuxing completed successfully: ${outputFile.length()} bytes, durationUs=$currentVideoPtsUs")
                    return true
                } catch (e: Exception) {
                    Log.e(TAG, "Error stopping muxer in HighlightVideoGenerator: ${e.message}", e)
                    return false
                }
            } else {
                Log.w(TAG, "Insufficient samples to safely stop muxer (video: $videoSamplesWritten, audio: $audioSamplesWritten)")
                return false
            }
        } catch (e: Exception) {
